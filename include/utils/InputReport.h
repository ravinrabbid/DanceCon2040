...
struct InputReport {
  private:
    ...
    usb_report_t getPS3DancePadInputReport(const InputState &state);
    usb_report_t getPS3InputReport(const InputState &state);
    usb_report_t getPS4InputReport(const InputState &state);
    usb_report_t getKeyboardReport(const InputState &state, Player player);
    usb_report_t getXinputReport(const InputState &state);
    usb_report_t getSpice2xReport(const InputState &state);
    usb_report_t getDebugReport(const InputState &state);

  public:
    InputReport() = default;

    usb_report_t getUsbReport(const InputState &state, usb_mode_t mode);
};
