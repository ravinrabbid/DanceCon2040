...
Controller::Button::Button(const uint8_t pin) : m_gpio_pin(pin), m_gpio_mask(1 << pin) {}

void Controller::Button::setState(const bool state, const uint8_t debounce_delay) {
    if (m_active == state) {
        return;
    }
...
void Controller::socdClean(Utils::InputState &input_state) {
    ...
    if (controller.dpad.left && controller.dpad.right) {
...
void Controller::updateInputState(Utils::InputState &input_state) {
    ...
    controller.dpad.right = m_buttons.at(Id::RIGHT).getState();
    controller.buttons.north = m_buttons.at(Id::NORTH).getState();
    controller.buttons.east = m_buttons.at(Id::EAST).getState();
    controller.buttons.south = m_buttons.at(Id::SOUTH).getState();
    controller.buttons.west = m_buttons.at(Id::WEST).getState();
    controller.buttons.l = m_buttons.at(Id::L).getState();
    controller.buttons.r = m_buttons.at(Id::R).getState();
    controller.buttons.start = m_buttons.at(Id::START).getState();
    controller.buttons.select = m_buttons.at(Id::SELECT).getState();
    controller.buttons.home = m_buttons.at(Id::HOME).getState();
    controller.buttons.share = m_buttons.at(Id::SHARE).getState();
